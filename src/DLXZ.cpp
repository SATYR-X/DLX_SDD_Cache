...
DancingLinks::DancingLinks( 
    ...
    const std::vector<std::vector<int>>& matrix
        ...
        sdd_false = sdd_manager_false(sdd_manager);
 ...
 * 2. 使用缓存优化搜索过程
 * 3. 构建表示所有解决方案的ZDD
 * 4. 这份代码已经把原来的ZDDNode*类型替换为SddNode*类型
 */
SddNode* DancingLinks::search()
{
    if(root->right == root){
        countSolution++;
        //return T;
    ...
    if (sdd_cache.find(columnState) != sdd_cache.end()) {
        countSolution++;
        //return C[columnState];
        // 修改为返回sdd_cache[columnState]
        return sdd_cache[columnState];
    }
